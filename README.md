...
- **Voice** — `scripts/voice.py` talks through the Mac's microphone and speakers with the OpenAI Realtime API (`gpt-realtime-2.1`) and mirrors each turn on the LED face (listen, think, speak); `--audio board` uses the ESP32 audio board's microphone and speaker over Wi-Fi instead. She waits for you to speak first and speaks Russian only (`--greet`, `--language`). Needs `uv`; stop with Ctrl-C or `scripts/voice.py --stop`.
- **Voice station** — `scripts/station.py` runs on the office Mac mini: the ESP32 audio board's microphone goes to the Hermes gateway's built-in speech-to-text, the question to the Burabai GIS assistant (`hermes_core` from `~/burabai-station`), and the answer through the Hermes text-to-speech server back to the board's speaker, with the turn mirrored on the LED face. No wake word; strictly half-duplex. `scripts/com.athena.station.plist` keeps it running under launchd
- **ESP32 boards** — face (`RGB-MATRIX.md`, on the ESP32-WROOM-32) and audio (`AUDIO-BOARD.md`, so far the raw microphone and amplifier bridge on an ESP32-S3), built with pure ESP-IDF through the global Claude Code `esp-idf` skill (`~/.claude/skills/esp-idf/`, not in this repo; Hermes does not load it). Athena-specific conventions are in `CLAUDE.md`.
