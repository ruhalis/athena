...
Hermes plugin. Sound is not this board's job: the microphone and the amplifier
are the separate ESP32-S3 audio board, `../athena_audio/`.
...
- The ESP32-S3 build uses the driver unchanged with a pin map that keeps every
  ...
  this is the bring-up path. Since 2026-09-10 the S3 on the desk is the audio
  board and the WROOM is the face for good, so that plan waits for another S3.
