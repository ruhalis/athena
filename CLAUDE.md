...
There is **no application code in this checkout**. Consequence: no build, no test suite, no linter, no package manifest. "Changing Athena's behavior" means editing prose in `HERMES.md` or a `SKILL.md`, or editing JSON/YAML declarations — not writing functions. The planned exception is ESP32-S3 firmware under `firmware/`, which does not exist yet (see "Firmware" below). When it does, it builds with `idf.py` under the global Claude Code `esp-idf` skill (`~/.claude/skills/esp-idf/`, personal, not in this repo, never loaded by Hermes).
...
**State of this checkout:** there is no `firmware/` directory, and no commit on any branch has ever contained one. `.gitmodules` declares the HUB75 driver submodule, but there is no gitlink and nothing is checked out, so `git submodule update --init` does nothing; materialize it with `git submodule add https://github.com/mrcodetastic/ESP32-HUB75-MatrixPanel-DMA firmware/athena_matrix/components/ESP32-HUB75-MatrixPanel-I2S-DMA` when the matrix project is created. `RGB-MATRIX.md` and `AUDIO-BOARD.md` are design notes to build against, not descriptions of current behavior; the Mac hub `athena.py` and the `display:` config they reference do not exist either.
...
- The HUB75 driver is `ESP32-HUB75-MatrixPanel-I2S-DMA` as a git submodule at the path above (its CMake expects that directory name), GFX off, behind one `.cpp` wrapper with `extern "C"` functions. Everything else is C.
