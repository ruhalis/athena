...
**Aura renderer.** The eight agent states are drawn by `firmware/athena_matrix/main/aura.c`, a C port of the aura shader from LiveKit's components-js (developed for Unicorn Studio, Polyform Non-Resale License 1.0.0, (c) 2026 UNCRN LLC); the notice in that file's header is a licence requirement, keep it. The shader runs on a 32x32 grid with 12 circle copies and 3 turbulence layers (36 and 4 upstream) on table sines and a fast square root, is upscaled bilinearly to 64x64, gets the haze, the error border and `t` at full resolution, and is quantised to the driver's 5-bit levels with a Bayer dither; it logs its draw time every 5 s. The per-state numbers and `firmware/athena_matrix/reference/aurora.js` (the JavaScript renderer the design canvas was rendered from; `render(mode, t, {look: "aura"})` is the reference for every number in `aura.c`) define the look; `face.c` only picks the mode and whether `t` is shown.
