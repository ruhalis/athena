...
- **`athena-face`** (`.hermes/plugins/athena-face/`) — the only project plugin. Registers Hermes hooks (`pre_gateway_dispatch`, `pre_llm_call`, `pre_api_request`, `pre_tool_call`, `post_tool_call`, `post_llm_call`, `api_request_error`, `pre_approval_request`, `post_approval_response`, `on_session_end`) and maps them to face states; hooks only enqueue, a daemon thread owns the serial port through `scripts/face.py` (stdlib only, the Hermes venv has no pyserial). Cron runs end in `alert` instead of `speak`. Env: `ATHENA_MATRIX_PORT`, `ATHENA_FACE_BRIGHTNESS`, `ATHENA_FACE_SLEEP`, `ATHENA_FACE=0`, `ATHENA_FACE_DRY_RUN=1`. Hermes discovers project plugins from the process cwd (`Path.cwd()/.hermes/plugins`), not from `terminal.cwd`, and the launchd-supervised gateway always runs with cwd `~/.hermes` (upstream pins the plist's `WorkingDirectory` there and regenerates the plist), so the checkout copy is invisible to it no matter where `hermes gateway restart` is run. Bootstrap therefore symlinks `~/.hermes/plugins/athena-face` to this checkout's copy, which makes it a *user* plugin that every Hermes process scans without a cwd or env gate (discovery dedupes by name, so a session started here still sees one entry), adds `athena-face` to `plugins.enabled`, and restarts a running gateway when the link is new. While a gateway runs it owns the serial port, so `scripts/face.py` reports a busy port then; use Telegram to exercise the face instead.
